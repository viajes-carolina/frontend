    ...
    >
      <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        ...
        <div className="text-center max-w-3xl mx-auto mb-12 sm:mb-16">
          ...
          <h2 className="font-sora font-extrabold text-3xl sm:text-4xl lg:text-[40px] leading-tight text-brand-navy mb-4">
            ...
            return (
              ...
              >
                ...
                <div className="flex flex-col min-w-0">
                  <span className={`font-sora font-bold text-sm sm:text-base leading-snug truncate ${isSelected ? "text-brand-navy" : "text-neutral-ink"}`}>
                    {item.title}
                  </span>
                  <span className="font-inter text-[11px] text-neutral-muted truncate">
        ...
        <div className="bg-white border border-neutral-border rounded-3xl p-6 sm:p-10 shadow-card">
          <div className="flex flex-col lg:flex-row items-center justify-between gap-8 lg:gap-12">
            ...
            <div className="w-full lg:w-1/2 flex flex-col items-start text-left space-y-6">
              ...
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h3 className="font-sora font-extrabold text-2xl sm:text-3xl text-brand-navy">
                    {current.title}
                  </h3>
                  <span className="font-inter text-xs text-brand-accent font-semibold uppercase tracking-wider">
              ...
              <div className="w-full pt-2">
                <span className="font-sora text-xs font-bold text-neutral-muted uppercase tracking-wider block mb-3">
                ...
                <div className="flex flex-wrap gap-2">
                  {current.featuredDestinations?.map((dest, dIdx) => (
                    <span
                      key={dIdx}
                      className="px-3.5 py-1.5 rounded-full bg-neutral-soft border border-neutral-border text-brand-navy font-inter text-xs font-medium hover:border-brand-accent/50 hover:bg-white transition-colors"
                    >
                      {dest}
                    </span>
                  ))}
              ...
              {/* Support note */}
              <div className="flex items-center gap-2 text-xs font-inter text-emerald-600 font-medium pt-1">
                <CheckIcon size={16} />
            ...
            <div className="w-full lg:w-1/2 flex justify-center lg:justify-end">
              <div className="relative w-full aspect-video sm:h-80 rounded-2xl overflow-hidden border border-neutral-border shadow-md group">
                {current.coverMediaUrl ? (
                  ...
                  <div className="w-full h-full bg-gradient-to-br from-brand-blue/20 to-brand-navy/10 flex flex-col items-center justify-center p-6">
                    <PlaneIcon size={40} className="text-brand-accent mb-2" />
                    <span className="font-sora text-sm text-brand-navy font-bold">{current.title}</span>
                  </div>
                )}

                {/* Bottom Overlay Info Tag */}
                <div className="absolute inset-x-4 bottom-4 flex items-center justify-between bg-brand-navy/90 backdrop-blur-md px-4 py-2.5 rounded-xl border border-white/15 text-xs text-white">
