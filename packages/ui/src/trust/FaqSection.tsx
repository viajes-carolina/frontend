      ...
      <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        ...
        <div className="text-center max-w-2xl mx-auto mb-12 sm:mb-16">
          ...
          <h2 className="font-sora font-extrabold text-3xl sm:text-4xl text-white mb-3">
            Preguntas Frecuentes sobre <span className="text-brand-sunset">tu próximo viaje</span>
          </h2>

          <p className="font-inter text-atmosphere-sky text-base leading-relaxed">
        ...
        <div className="space-y-4">
          {faqs.map((faq, idx) => {
            ...
            return (
              <div
                key={faq.id || idx}
                className="bg-atmosphere-twilight/80 border border-white/15 rounded-2xl overflow-hidden transition-all duration-300 hover:border-white/30"
              >
                <button
                  type="button"
                  onClick={() => toggleIndex(idx)}
                  className="w-full px-6 py-5 flex items-center justify-between gap-4 text-left font-sora font-semibold text-base sm:text-lg text-white hover:text-brand-accent transition-colors"
                >
                ...
                {isOpen && (
                  <div className="px-6 pb-6 pt-1 border-t border-white/10 font-inter text-sm text-atmosphere-sky leading-relaxed animate-fade-in text-left">
                    <p>{faq.answer}</p>
                  </div>
                )}
              </div>
        ...
        <div className="mt-12 p-6 sm:p-8 rounded-3xl bg-gradient-to-r from-brand-blue/30 to-brand-navy border border-white/15 flex flex-col sm:flex-row items-center justify-between gap-6 text-center sm:text-left">
          <div>
