  ...
  if (!config) {
    return (
      <div className="bg-red-50 text-red-700 p-4 rounded-xl border border-red-200">
        No se pudo cargar los términos y condiciones.
      </div>
    );
  }
  ...
  return (
    <form onSubmit={handleSubmit} className="bg-white rounded-2xl p-8 border border-neutral-border shadow-sm space-y-6 max-w-4xl">
      <div className="border-b border-neutral-border pb-4">
        <h2 className="text-xl font-bold text-brand-navy">Términos y condiciones</h2>
        <p className="text-xs text-neutral-muted mt-1">Ruta pública: /terminos</p>
      </div>
      ...
      <div className="flex justify-end pt-4 border-t border-neutral-border">
        <Button variant="primary" type="submit" disabled={saving}>
          {saving ? "Guardando..." : "Guardar Términos y condiciones"}
        </Button>
      </div>
    </form>
  );
